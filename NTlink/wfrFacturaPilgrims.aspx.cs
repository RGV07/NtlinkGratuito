...
{
    public partial class wfrFacturaPilgrims : Page
    {
        ...
        //        ddlClaveUnidadE.Style.Remove("width");
                ...
                try
                {
                    ...
                    ViewState["conPilgrims"] = new List<DatosPilgrims>();
    ...
    protected void bntEliminarImpuesto_Click(object sender, EventArgs e)
        ...
        private facturas GetFactura(string iniciales, List<facturasdetalle> detalles)
        {
             ...
             */
            fact.AddendaPilgrims = GetDatosFacturaAduana();
        ...
        private Pilgrims GetDatosFacturaAduana()
        {

            //Pilgrims
            var addendaPilgrims = new Pilgrims();
            addendaPilgrims.Proveedor = txtProveedor.Text;
            addendaPilgrims.Comprador = (string.IsNullOrEmpty(txtComprador.Text) ? null : txtComprador.Text);
            addendaPilgrims.Proceso = ddlProceso.SelectedValue;

            var partidas = new List<PilgrimsPartida>();
            var DP = ViewState["conPilgrims"] as List<DatosPilgrims>;

            foreach (DatosPilgrims dp in DP)
            {
                PilgrimsPartida partida = new PilgrimsPartida();
                //partida.
                partida.Referencia = dp.Referencia;
                partida.Posicion = dp.Posicion;
                partida.Pedido = dp.Pedido;
                partida.Pedimento = (string.IsNullOrEmpty(dp.Pedimento) ? null : dp.Pedimento);
                partida.FacturaPedimento = (string.IsNullOrEmpty(dp.FacturaPedimento) ? null : dp.FacturaPedimento);

                partidas.Add(partida);
            }
            addendaPilgrims.Partida = partidas.ToArray();

            return addendaPilgrims;
        }
        ...
        {
            var conceptos = ViewState["conPilgrims"] as List<DatosPilgrims>;
            if (conceptos != null && conceptos.Count > 0)
            {
        ...
        protected void btnAgregarPilgrims_Click(object sender, EventArgs e)
        {
            ...
            if (DP == null)
            {
                DP = new List<DatosPilgrims>();
                ViewState["conPilgrims"] = DP;
            }
            var p = new DatosPilgrims
            {

                Pedimento = txtPedimento.Text,
                Pedido = txtPedidoConcepto.Text,
                Referencia = txtReferencia.Text,
                Posicion = txtPosicion.Text,
                FacturaPedimento = txtFacturaPedimento.Text,
            };

            DP.Add(p);
            ViewState["conPilgrims"] = DP;
            txtPedimento.Text = "";
            txtPedidoConcepto.Text = "";
            txtReferencia.Text = "";
            txtPosicion.Text = "";
            txtFacturaPedimento.Text = "";
            BindPilgrimsToGridView();
        }
