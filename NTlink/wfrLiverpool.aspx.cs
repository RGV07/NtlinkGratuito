...
{
    public partial class wfrLiverpool : Page
    {
        ...
        private void ClearAll()
        {
            ...
            this.txtProyecto.Text = this.txtFolioSat.Text =
    ...
    protected void bntEliminarImpuesto_Click(object sender, EventArgs e)
        ...
        private facturas GetFactura(string iniciales, List<facturasdetalle> detalles)
        {
            ...
            {
                TipoDocumentoSAT = TipoDocumento.Pilgrims,
                TipoDocumento = TipoDocumento.Pilgrims,
            ...
            request.documentStatus = detallistaDocumentStatus.ORIGINAL;
            request.documentStructureVersion = "AMC8.1";
            request.requestForPaymentIdentification = new detallistaRequestForPaymentIdentification();
            request.requestForPaymentIdentification.entityType = detallistaRequestForPaymentIdentificationEntityType.INVOICE;
            request.specialInstruction = new[]
                                             {
                                                 new detallistaSpecialInstruction()
                                                 {
                                                     code = detallistaSpecialInstructionCode.ZZZ,
                                                     text = new[] {CantidadLetra.Enletras(fact.Total.ToString(), "MXN")}
                                                 },
                                             };
            request.orderIdentification = new detallistaOrderIdentification();
            request.orderIdentification.ReferenceDate = fact.Fecha;
            request.orderIdentification.ReferenceDateSpecified = true;
            ...
            request.AdditionalInformation = new[] {new detallistaReferenceIdentification
                                                       {
                                                           type = detallistaReferenceIdentificationType.ACE,
                                                           Value = "1"
                                                       }};
            request.DeliveryNote = new detallistaDeliveryNote();
            request.DeliveryNote.ReferenceDate = DateTime.Now;
            request.DeliveryNote.ReferenceDateSpecified = true;
            request.DeliveryNote.referenceIdentification = new[] { txtDeliveryNote.Text };
            request.buyer = new detallistaBuyer();
            request.buyer.gln = BuyerSeller_GlnBuyer.Text;
            request.buyer.contactInformation = new detallistaBuyerContactInformation();
            request.buyer.contactInformation.personOrDepartmentName = new detallistaBuyerContactInformationPersonOrDepartmentName();
            request.buyer.contactInformation.personOrDepartmentName.text = txtpersonOrDepartmentName.Text;

            request.seller = new detallistaSeller();
            request.seller.gln = txtSellerGln.Text;
            request.seller.alternatePartyIdentification = new detallistaSellerAlternatePartyIdentification();
            request.seller.alternatePartyIdentification.type = detallistaSellerAlternatePartyIdentificationType.SELLER_ASSIGNED_IDENTIFIER_FOR_A_PARTY;
            request.seller.alternatePartyIdentification.Value = txtalternatePartyIdentification.Text;


            request.allowanceCharge = new[]
                                          {
                                              new detallistaAllowanceCharge()
                                              {
                                                  ...
                                                  monetaryAmountOrPercentage = new detallistaAllowanceChargeMonetaryAmountOrPercentage()
                                                                               {
            ...
            {
                if (comprobanteConcepto.Descripcion.Length > 35)
                    longTexto = comprobanteConcepto.Descripcion.Substring(0, 35);
                else
                    longTexto = comprobanteConcepto.Descripcion;

                detallistaLineItem li = new detallistaLineItem();
                li.type = "SimpleInvoiceLineItemType";
                li.number = (++countConceptos).ToString();

                li.tradeItemIdentification = new detallistaLineItemTradeItemIdentification()
                {
                ...
                li.alternateTradeItemIdentification = new[]
                                                          {
                                                              new detallistaLineItemAlternateTradeItemIdentification()
                                                              {
                                                                  type =
                                                                      ...
                                                                      .BUYER_ASSIGNED,
                                                                  Text = new[] {comprobanteConcepto.Codigo}
                                                              }
                                                          };
                li.tradeItemDescriptionInformation = new detallistaLineItemTradeItemDescriptionInformation()
                {
                ...
                li.invoicedQuantity = new detallistaLineItemInvoicedQuantity();
                li.invoicedQuantity.Text = new[] { comprobanteConcepto.Cantidad.ToString() };
                li.invoicedQuantity.unitOfMeasure = comprobanteConcepto.Unidad;
            ...
            request.TotalAllowanceCharge = new detallistaTotalAllowanceCharge[]
                                                   {

                                                       new detallistaTotalAllowanceCharge()
                                                           {
                                                               specialServicesType = detallistaTotalAllowanceChargeSpecialServicesType.AA,
                                                               specialServicesTypeSpecified = true,
                                                               Amount = 0, AmountSpecified = true,allowanceOrChargeType = detallistaTotalAllowanceChargeAllowanceOrChargeType.ALLOWANCE

                                                           }
                                                   };
            request.totalAmount = new detallistaTotalAmount()
            {
                Amount = fact.Total.Value
            };

            fact.Detallista = request;
            //--------------------------
