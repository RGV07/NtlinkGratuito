...
{
    public partial class wfrCartaPorte : Page
    {
        #region Definicion
        private const String AddendaConcepto = "AddendaConcepto";
        #endregion
        ...
        {
            ViewState[AddendaConcepto] = new List<ConceptosCartaPorte>();
            this.Clear();
        ...
        private facturas GetFactura(string iniciales, List<facturasdetalle> detalles)
        {
             ...
             */
            //---------------------CARTA PORTE--------------------------
            var listaconceptos = ViewState[AddendaConcepto] as List<ConceptosCartaPorte>;

            if (listaconceptos != null)
            {
                fact.ConceptosCartaPortes = new List<ConceptosCartaPorte>();
                foreach (var conceptoCartaPorte in listaconceptos)
                {
                    fact.ConceptosCartaPortes.Add(conceptoCartaPorte);
                }
            }
        ...
        protected void gvDescripcionAddenda_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        {
            if (ViewState[AddendaConcepto] == null)
                ViewState[AddendaConcepto] = new List<ConceptosCartaPorte>();
            var conceptosAduana = ViewState[AddendaConcepto] as List<ConceptosCartaPorte>;
            var cartaPorte = new ConceptosCartaPorte
            {
                Descripcion = txtAddendaDescripcion.Text,
                Valor = txtAddendaValor.Text
            };

            conceptosAduana.Add(cartaPorte);
            ViewState[AddendaConcepto] = conceptosAduana;

            txtAddendaDescripcion.Text = String.Empty;
            txtAddendaValor.Text = String.Empty;

            UpdateAddendaGridView();
        }
        protected void btnBorrarConceptosAddenda0_Click(object sender, EventArgs e)
        {
            var conceptosAduana = ViewState[AddendaConcepto] as List<ConceptosCartaPorte>;
            conceptosAduana.Clear();
            UpdateAddendaGridView();
        }
        private void UpdateAddendaGridView()
        {
            this.gvDescripcionAddenda.DataSource = ViewState[AddendaConcepto];
            this.gvDescripcionAddenda.DataBind();
        }
        protected void gvDescripcionAddenda_OnRowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Borrar")
            {
                var addedaDescripcion = ViewState[AddendaConcepto] as List<ConceptosCartaPorte>;
                addedaDescripcion.RemoveAt(Convert.ToInt32(e.CommandArgument));
                ViewState[AddendaConcepto] = addedaDescripcion;
                UpdateAddendaGridView();
            }
        }
