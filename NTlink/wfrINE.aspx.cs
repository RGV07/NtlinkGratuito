...
{
    public partial class wfrINE : Page
    {
        ...
        //        ddlClaveUnidadE.Style.Remove("width");
                ...
                try
                {
                    ...
                    ViewState["INEEntidad"] = new List<INEEntidad>();
                    ViewState["INEContabilidad"] = new List<INEContabilidad>();
            ...
            if (ValidarFactura())
            {
                ...
                try
                {
                    ...
                    using (clienteServicio as IDisposable)
                    {
                        ...
                        List<INEEntidad> INEEntidad = ViewState["INEEntidad"] as List<INEEntidad>;
                        List<INEContabilidad> INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;


                        INE ine = new INE();
                        ine.IdContabilidad = txtIdContabilidad.Text;
                        ine.TipoComite = ddlTipoComite.SelectedValue;
                        if (ine.TipoComite == "0")
                            ine.TipoComite = null;
                        ine.TipoProceso = ddlTipoProc.SelectedValue;
                        ine.Entidad = INEEntidad;
                        foreach (var EN in ine.Entidad)
                        {
                            foreach (var Cont in INEContabilidad)
                            {
                                if (EN.ID == Cont.ID)
                                {
                                    if (EN.IdContabilidad == null)
                                        EN.IdContabilidad = new List<string>();
                                    EN.IdContabilidad.Add(Cont.IdContabilidad);
                                }
                            }
                        ...
                        if (ine != null)
                        {
                            complementos.ine = new INE();
                            complementos.ine = ine;
                        }
                        //------------------------------------------------
        ...
        private void ClearAll()
        {
            ...
            //----------INE-----------------------
            var INE = new List<INEEntidad>();
            ViewState["INEEntidad"] = INE;
            var Contabilidad = new List<INEContabilidad>();
            ViewState["INEContabilidad"] = Contabilidad;
            //-------------------------------------
    ...
    protected void bntEliminarImpuesto_Click(object sender, EventArgs e)
        ...
        //------------------------------INE---------------------------------------------
        private void BindEntidadesToGridView()
        {
            List<INEEntidad> INEEntidad = ViewState["INEEntidad"] as List<INEEntidad>;
            if (INEEntidad != null && INEEntidad.Count > 0)
            {
                int noColumns = this.gvEntidad.Columns.Count;
                //int noColumns = 0;
                this.gvEntidad.Columns[noColumns - 1].Visible = this.gvEntidad.Columns[noColumns - 2].Visible = true;
            }
            else
            {
                int noColumns = this.gvEntidad.Columns.Count;
                this.gvEntidad.Columns[noColumns - 1].Visible = this.gvEntidad.Columns[noColumns - 2].Visible = false;
            }


            this.gvEntidad.DataSource = INEEntidad;
            this.gvEntidad.DataBind();
        }
        private void BindIdContabilidadEntidadesToGridView()
        {
            List<INEContabilidad> INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;

            if (INEContabilidad != null && INEContabilidad.Count > 0)
            {
                int noColumns = this.dvIdContabilidad.Columns.Count;
                //int noColumns = 0;
                this.dvIdContabilidad.Columns[noColumns - 1].Visible = this.dvIdContabilidad.Columns[noColumns - 2].Visible = true;
            }
            else
            {
                int noColumns = this.dvIdContabilidad.Columns.Count;
                this.dvIdContabilidad.Columns[noColumns - 1].Visible = this.dvIdContabilidad.Columns[noColumns - 2].Visible = false;
            }


            this.dvIdContabilidad.DataSource = INEContabilidad;
            this.dvIdContabilidad.DataBind();
        }

        protected void btnAgregarEntidad_Click(object sender, EventArgs e)
        {
            //if (ddlAmbito.SelectedValue != "0" && ddlClaveEntidad.SelectedValue != "0")
            if (ddlClaveEntidad.SelectedValue != "0")
                if (!existe_convinacion())

                {
                    List<INEEntidad> INEEntidad = ViewState["INEEntidad"] as List<INEEntidad>;
                    int t = INEEntidad.Count() + 1;
                    INEEntidad entidad = new INEEntidad();
                    if (ddlAmbito.SelectedValue != "0")
                        entidad.Ambito = ddlAmbito.SelectedValue;
                    else
                        entidad.Ambito = "";
                    entidad.ClaveEntidad = ddlClaveEntidad.SelectedValue;
                    entidad.ID = t.ToString();
                    ddlIDContabilidad.Items.Add(t.ToString());
                    INEEntidad.Add(entidad);
                    //ddlID.Items.Add(t.ToString());
                    ViewState["INEEntidad"] = INEEntidad;
                    this.BindEntidadesToGridView();
                    //txtIdContabilidad.Text = "";
                }
                else
                    lblErrorINE.Text = "Relación 'Ambito - Entidad' ya agregada";
            else
                lblErrorINE.Text = "Seleccione una Relación Ambito - Entidad.";
        }

        protected void gvEntidad_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("EliminarEntidad"))
            {
                var INEEntidad = ViewState["INEEntidad"] as List<INEEntidad>;
                var INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;

                INEEntidad ET = INEEntidad.ElementAt(Convert.ToInt32(e.CommandArgument));
                string x = ET.ID;

                INEEntidad.RemoveAt(Convert.ToInt32(e.CommandArgument));
                ViewState["INEEntidad"] = INEEntidad;
                ddlIDContabilidad.Items.Remove(x);


                INEContabilidad.RemoveAll(p => p.ID == x);
                ViewState["INEContabilidad"] = INEContabilidad;
                this.BindEntidadesToGridView();
                this.BindIdContabilidadEntidadesToGridView();
        ...
        protected void ajustar_ine()
        {
            ...
            if (ddlTipoProc.SelectedValue == "0")
            {
                tr1.Visible = false;
                tr2.Visible = false;
                tblentidad.Visible = false;
                //-----------------------------------
                ddlTipoComite.SelectedIndex = 0;
                txtIdContabilidad.Text = null;
                ddlAmbito.SelectedIndex = 0;
                tdddlentidad.Visible = false;
                tdlblentidad.Visible = false;
                BindEntidad(ddlAmbito.SelectedValue);
                //-----------------------------------
            }
            else if (ddlTipoProc.SelectedValue == "Ordinario")
            {
                tr1.Visible = true;
                tr2.Visible = false;
                tblentidad.Visible = false;
                if (ddlTipoComite.SelectedValue == "0")
                {
                    tdlblcontabilidad.Visible = false;
                    tdtxtcontabilidad.Visible = false;
                    tr2.Visible = false;
                    tblentidad.Visible = false;
                    //-----------------------------------
                    txtIdContabilidad.Text = null;
                ...
                else if (ddlTipoComite.SelectedValue == "Ejecutivo Nacional")
                {
                    tdlblcontabilidad.Visible = true;
                    tdtxtcontabilidad.Visible = true;
                    //-----------------------------------
                    ddlAmbito.SelectedIndex = 0;
                    tdddlentidad.Visible = false;
                    tdlblentidad.Visible = false;
                ...
                {
                    if (ddlTipoComite.SelectedValue == "Directivo Estatal")
                    {
                        tdlblcontabilidad.Visible = true;
                        tdtxtcontabilidad.Visible = true;
                    }
                    ...
                    tr2.Visible = true;
                    tblentidad.Visible = true;
                    //-----------------------------------
                    txtIdContabilidad.Text = null;
                    tdddlentidad.Visible = true;
                    tdlblentidad.Visible = true;
                    BindEntidad("Local");

                    //-----------------------------------
                }
            }
            else if (ddlTipoProc.SelectedValue == "Campaña" || ddlTipoProc.SelectedValue == "Precampaña")
            {
                tr2.Visible = true;
                tblentidad.Visible = true;
                tr1.Visible = false;
        ...
        protected bool existe_convinacion()
        {
            int cont = 0;
            List<INEEntidad> INEEntidad = ViewState["INEEntidad"] as List<INEEntidad>;
            foreach (var x in INEEntidad)
            {
                if (x.Ambito == ddlAmbito.SelectedValue && x.ClaveEntidad == ddlClaveEntidad.SelectedValue)
                {
                    cont++;
                }
            }
        ...
        protected void ddlTipoProc_SelectedIndexChanged(object sender, EventArgs e)
        {
            ajustar_ine();
        }

        protected void ddlTipoComite_SelectedIndexChanged(object sender, EventArgs e)
        {
            ajustar_ine();
        }
        ...
        protected void BindEntidad(string ambito)
        {
            string[,] Paq = new string[,]{{ "AGU","BCN","BCS","CAM","CHP","CHH","COA","COL","DIF",
                    "DUR","GUA","GRO","HID","JAL","MEX","MIC","MOR","NAC","NAY","NLE","OAX","PUE",
                    "QUE","ROO","SLP","SIN","SON","TAB","TAM","TLA","VER","YUC","ZAC","CR1","CR2",
                    "CR3","CR4","CR5"},{"Aguascalientes - AGU","Baja California - BCN","Baja California Sur - BCS",
                    "Campeche - CAM","Chiapas - CHP","Chihuahua - CHH","Coahuila - COA","Colima - COL","Ciudad de México - DIF",
                    "Durango - DUR","Guanajuato - GUA","Guerrero - GRO","Hidalgo - HID","Jalisco - JAL","México - MEX","Michoacán - MIC",
                    "Morelos - MOR","Nacional - NAC","Nayarit - NAY","Nuevo León - NLE","Oaxaca - OAX","Puebla - PUE","Querétaro - QUE",
                    "Quintana Roo - ROO","San Luis Potosí - SLP","Sinaloa - SIN","Sonora - SON","Tabasco - TAB","Tamaulipas - TAM",
                    "Tlaxcala - TLA","Veracruz - VER","Yucatán - YUC","Zacatecas - ZAC","Circunscripción 1 - CR1",
                    "Circunscripción 2 - CR2","Circunscripción 3 - CR3","Circunscripción 4 - CR4","Circunscripción 5 - CR5"} };
            System.Data.DataTable workTable = new System.Data.DataTable();
            workTable.Columns.Add("Valor", typeof(String));
            workTable.Columns.Add("Texto", typeof(String));
            ...
            this.ddlClaveEntidad.DataSource = workTable;
            this.ddlClaveEntidad.DataTextField = workTable.Columns[1].ToString();
            this.ddlClaveEntidad.DataValueField = workTable.Columns[0].ToString();
            this.ddlClaveEntidad.DataBind();
        }

        protected void ddlClaveEntidad_SelectedIndexChanged(object sender, EventArgs e)
        {
            lblErrorINE.Text = "";
        }
        //------------------------------------------------------------------------------
        protected void btnAgregarIDContabilidadEntidad_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(ddlIDContabilidad.SelectedValue) && !string.IsNullOrEmpty(txtIDContabilidadEntidad.Text))
            {
                List<INEContabilidad> INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;
                INEContabilidad IdCont = new INEContabilidad();
                IdCont.ID = ddlIDContabilidad.SelectedValue;
                IdCont.IdContabilidad = txtIDContabilidadEntidad.Text;
                INEContabilidad.Add(IdCont);
                ViewState["INEContabilidad"] = INEContabilidad;
                txtIDContabilidadEntidad.Text = "";
                this.BindIdContabilidadEntidadesToGridView();
        ...
        protected void dvIdContabilidad_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("EliminarIdContabilidad"))
            {
                List<INEContabilidad> INEContabilidad = ViewState["INEContabilidad"] as List<INEContabilidad>;

                INEContabilidad ET = INEContabilidad.ElementAt(Convert.ToInt32(e.CommandArgument));
                string x = ET.ID;

                INEContabilidad.RemoveAt(Convert.ToInt32(e.CommandArgument));
                ViewState["INEContabilidad"] = INEContabilidad;
                this.BindIdContabilidadEntidadesToGridView();
                //this.UpdateTotales();
        ...
        protected void ddlIDContabilidad_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        protected void txtIDContabilidadEntidad_TextChanged(object sender, EventArgs e)
        {
