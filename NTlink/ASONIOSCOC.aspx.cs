...
{
    public partial class ASONIOSCOC : Page
    {
        ...
        //        ddlClaveUnidadE.Style.Remove("width");
                ...
                try
                {
                    ...
                    ViewState["AtributosASONIOSCOC"] = new List<ASONIOSCOCLosAtributos>();
        ...
        private void BindAddendaToGridView()
        {
            var conceptos = ViewState["AtributosASONIOSCOC"] as List<ASONIOSCOCLosAtributos>;
            if (conceptos != null && conceptos.Count > 0)
            {
                int noColumns = this.gvAmazon.Columns.Count;
                this.gvAmazon.Columns[noColumns - 1].Visible = true;

            }
            else
            {
                int noColumns = this.gvAmazon.Columns.Count;
                this.gvAmazon.Columns[noColumns - 1].Visible = false;

            }
            this.gvAmazon.DataSource = ViewState["AtributosASONIOSCOC"];
            this.gvAmazon.DataBind();
        }
    ...
    protected void bntEliminarImpuesto_Click(object sender, EventArgs e)
        ...
        private facturas GetFactura(string iniciales, List<facturasdetalle> detalles)
        {
            ...
            {
                TipoDocumentoSAT = TipoDocumento.ASONIOSCOC,
                TipoDocumento = TipoDocumento.ASONIOSCOC,
             ...
             */
            fact.FacturasAddenASONIOSCOC = GetDatosFacturaAddenda();
        ...
        private DatosASONIOSCOC GetDatosFacturaAddenda()
        {
            //campos txt factura addenda ASONIOSCOC
            DatosASONIOSCOC dfa = new DatosASONIOSCOC();
            dfa.noProveedor = txtNoProveedor.Text;
            dfa.tipoProveedor = txtTipoProveedor.Text;
            dfa.ordenCompra = txtOrdenCompra.Text;
            dfa.serie = txtSerie.Text;
            dfa.folio = txtFolio.Text;

            var atributos = ViewState["AtributosASONIOSCOC"] as List<ASONIOSCOCLosAtributos>;
            dfa.atributos = atributos;
        ...
        protected void gvAmazon_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "EliminarAtributo")
            {
                var atributos = ViewState["AtributosASONIOSCOC"] as List<ASONIOSCOCLosAtributos>;
                atributos.RemoveAt(Convert.ToInt32(e.CommandArgument));
                ViewState["AtributosASONIOSCOC"] = atributos;
                this.BindAddendaToGridView();
        ...
        {
            if (string.IsNullOrEmpty(this.txtNoPartida.Text.Trim()) ||
              string.IsNullOrEmpty(this.txtIvaAcreditable.Text.Trim()) ||
              string.IsNullOrEmpty(this.txtIvaDevengado.Text.Trim()) ||
              string.IsNullOrEmpty(this.txtOtros.Text.Trim())
              )
            {
                this.lblError.Text = "* Error de validación en los atributos de la addenda";
                return;
            }
            this.lblError.Text = string.Empty;

            var atributos = ViewState["AtributosASONIOSCOC"] as List<ASONIOSCOCLosAtributos>;
            var atributo = new ASONIOSCOCLosAtributos();

            atributo.noPartida = txtNoPartida.Text;
            atributo.ivaAcreditable = txtIvaAcreditable.Text;
            atributo.ivaDevengado = txtIvaDevengado.Text;
            atributo.Otros = txtOtros.Text;

            atributos.Add(atributo);
            ViewState["AtributosASONIOSCOC"] = atributos;
            txtNoPartida.Text = "";
            txtIvaAcreditable.Text = "";
            txtIvaDevengado.Text = "";
            txtOtros.Text = "";
            this.BindAddendaToGridView();
        }
