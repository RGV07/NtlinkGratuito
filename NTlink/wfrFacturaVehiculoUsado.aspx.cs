...
{
    public partial class wfrFacturaVehiculoUsado : Page
    {
            ...
            if (ValidarFactura())
            {
                ...
                try
                {
                    ...
                    using (clienteServicio as IDisposable)
                    {
                        ...
                        var ss = clienteServicio.GuardarFactura33(fact, detalles, fact33, true, GetDatosFacturaAduana(), null);
                        if (!ss.resultado)
                        {
        ...
        private facturaComplementos GetDatosFacturaAduana()
        {

            var addendaVehiculo = new VehiculoUsado();
            addendaVehiculo.claveVehicular = txtClaveVehicular.Text;
            addendaVehiculo.marca = txtMarca.Text;
            addendaVehiculo.modelo = txtModelo.Text;
            addendaVehiculo.montoAdquisicion = Convert.ToDecimal(txtmontoAdquisicion.Text);
            addendaVehiculo.montoEnajenacion = Convert.ToDecimal(txtMontoEnajenacion.Text);
            addendaVehiculo.NIV = txtNIV.Text;
            addendaVehiculo.numeroMotor = txtNumeroMotor.Text;
            addendaVehiculo.numeroSerie = txtNumeroSerie.Text;
            addendaVehiculo.tipo = txtTipo.Text;
            addendaVehiculo.valor = Convert.ToDecimal(txtValor.Text);
            addendaVehiculo.Version = "1.0";


            var partidas = new List<ServicioLocalContract.t_InformacionAduanera>();
            var DP = ViewState["InformacionVehiculoUsado"] as List<VUInformacionAduanera>;

            foreach (VUInformacionAduanera dp in DP)
            {
                ServicioLocalContract.t_InformacionAduanera partida = new ServicioLocalContract.t_InformacionAduanera();
                //partida.
                partida.aduana = dp.aduana;
                DateTime temp = DateTime.ParseExact(dp.fecha, "dd/MM/yyyy", new CultureInfo("es-MX"));
                string str = temp.ToString("yyyy-MM-dd");
                partida.fecha = str;
                partida.numero = dp.numero;

                partidas.Add(partida);
            }
            addendaVehiculo.InformacionAduanera = partidas.ToArray();

            facturaComplementos compl = new facturaComplementos();
            compl.vehiculoUsado = addendaVehiculo;
    ...
    protected void bntEliminarImpuesto_Click(object sender, EventArgs e)
        ...
        private byte[] Preview()
        {
            ...
            try
            {
                ...
                using (clienteServicio as IDisposable)
                {
                    ...
                    var pdf = clienteServicio.FacturaPreview33(fact, detalles, fact33, GetDatosFacturaAduana(), null);
                    if (pdf == null)
                    {
                        this.lblError.Text = "* Error al generar la factura";
                        mpMensajeError.Show();
        ...
        private facturas GetFactura(string iniciales, List<facturasdetalle> detalles)
        {
            ...
            {
                TipoDocumentoSAT = TipoDocumento.FacturaGeneral,
        ...
        protected void btnAgregarPilgrims_Click(object sender, EventArgs e)
        {
            var DP = ViewState["InformacionVehiculoUsado"] as List<VUInformacionAduanera>;
            if (DP == null)
            {
                DP = new List<VUInformacionAduanera>();
                ViewState["InformacionVehiculoUsado"] = DP;
            }
            var p = new VUInformacionAduanera
            {
                numero = txtNumero.Text,
                fecha = txtFechaVU.Text,
        ...
        private void BindPilgrimsToGridView()
        {
            var conceptos = ViewState["InformacionVehiculoUsado"] as List<VUInformacionAduanera>;

            if (conceptos != null && conceptos.Count > 0)
            {
                int noColumns = this.gvPilgrims.Columns.Count;
                this.gvPilgrims.Columns[noColumns - 1].Visible = this.gvPilgrims.Columns[noColumns - 2].Visible = true;
            }
            else
            {
                int noColumns = this.gvPilgrims.Columns.Count;
                this.gvPilgrims.Columns[noColumns - 1].Visible = this.gvPilgrims.Columns[noColumns - 2].Visible = false;
            }
            this.gvPilgrims.DataSource = conceptos;
            this.gvPilgrims.DataBind();
        }

        protected void gvPilgrims_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "EliminarPilgrims")
            {
                var DP = ViewState["InformacionVehiculoUsado"] as List<VUInformacionAduanera>;
                DP.RemoveAt(Convert.ToInt32(e.CommandArgument));
                this.gvPilgrims.DataSource = DP;
                this.gvPilgrims.DataBind();
