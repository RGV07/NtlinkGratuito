...
namespace ServicioLocal.Business
{
    public class NtLinkEmpresa : NtLinkBusiness
    {
            ...
            try
            {
                ...
                if (certificado != null)
                {
                    if (cer.SerialNumber != null)
                    {
                        ...
                        Operaciones_IRFC lco = new Operaciones_IRFC();
                        vLCO valor= lco.SearchLCOByNoCertificado(serialNumber);
        ...
        //public RfcLco CSDValidarxx(string cadena)
        //{
        //    try
        //    {
        //        using (var db = new NtLinkLocalServiceEntities())
        ...
        public bool UpdateTimbresEmpresas(int id, Int64 WSTimbresAsigados, Int64 WSTimbresConsumidos)
        {
            try
            {
                using (var db = new NtLinkLocalServiceEntities())
                {
                    ...
                    var y = db.empresa.Where(p => p.IdEmpresa == id).FirstOrDefault();
                   // y.WSTimbresAsigados = WSTimbresAsigados;
                  //  y.WSTimbresConsumidos = WSTimbresConsumidos;
                    db.empresa.ApplyCurrentValues(y);
                    db.SaveChanges();
                    return true;
                }
            }
