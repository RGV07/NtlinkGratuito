    ...
    public class NtLinkClientes : NtLinkBusiness
    {
        ...
        public Promotores ObtenerPromotores(int idPromotor)
        {
            try
            {
                using (var db = new NtLinkLocalServiceEntities())
                {
                    var ObtenerPromotor = db.Promotores.FirstOrDefault(p => p.IdPromotor == idPromotor);
        ...
        public clientes GetClientePorEmpresa(string rfc, int idEmpresa)
        {
            ...
            try
            {
                using (var db = new NtLinkLocalServiceEntities())
                {
                    var cliente = db.clientes.Where(c => c.RFC == rfc && c.idempresa==idEmpresa);
                    return cliente.FirstOrDefault();
                }
            }
            catch (Exception ee)
