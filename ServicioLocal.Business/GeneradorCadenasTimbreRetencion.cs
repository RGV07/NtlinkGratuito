...
namespace ServicioLocal.Business
{
    class GeneradorCadenasTimbreRetencion
    {
        ...
        public GeneradorCadenasTimbreRetencion(bool timbre)
        {
            if (xsl == null)
            {
                ...
                try
                {
                    xsl = File.ReadAllText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Xslt\cadenaoriginal_TFD_1_0.xslt"));
        ...
        public GeneradorCadenasTimbreRetencion()
        {
            if (xsl == null)
            {
                ...
                try
                {
                    xsl = File.ReadAllText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Resources\cadenaoriginal_TFD_1_0.xslt"));
